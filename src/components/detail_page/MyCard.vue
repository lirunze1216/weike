<template>
  ...
  <!-- phone 就是item -->
  <div :key="index" v-for="(phone, index) in cart">
    <van-swipe-cell>
      <div class="message" style="touch-action: none">
        <input type="checkbox" name="select" :value="phone" v-model="select_list" class="checked" />
        <!-- 类名是 van-card -->
        <van-card :price="phone.price" :thumb="phone.thumb" :title="phone.title" :num="phone.number"></van-card>
      </div>
      <template #right>
        <van-button square text="删除" type="danger" @click="delete_item(index)" style="height: 100%" />
      </template>
    </van-swipe-cell>
    ...
    <div class="footer">
      <van-tabbar v-model="active" class="bottom_nav">
        ...
        <van-button text="提交订单" @click="onSub" />
      </van-tabbar>
    </div>
  </div>
</template>
